...
urlpatterns = patterns(
    ...
    url(r'^export_pad/(?P<pk>[0-9]+)$', login_required(export_pad), name='export_pad'),
    url(r'^(?P<pk>[0-9]+)', EventDetailView.as_view(), name='view_event'),
    url(r'^add_meeting/(?P<pk>[0-9]+)', login_required(MeetingAddView.as_view()), name='add_meeting'),
    url(r'^edit_meeting/(?P<pk>[0-9]+)', login_required(MeetingEditView.as_view()), name='edit_meeting'),
